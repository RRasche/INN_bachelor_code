...
from old import INNOld #contains the classes to build an INN
...
def fourGauss(save = False):
    ...
    #creating an INN
    inn = INNOld(4,4,latent_dim,label_dim,256,input_dim,2.0,[2, 8, 9, 5, 6, 3, 7, 1, 4, 0],zero_noise,y_noise,0.1)
    ...
    #compiling the model.
    inn.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=0.0002, beta_1=0.99, beta_2=0.999,amsgrad=False,clipvalue=15.0),loss=[losses.MMD_forward,'mse','mse',losses.MMD_backward],loss_weights=[300,1,1,350],metrics=['accuracy'])
